...
def integrateGraph(training_user_set, mini_user_set):

  training_item_set = collections.defaultdict(set)

  for k,v in mini_user_set.items():
    training_user_set[k] = training_user_set[k] | mini_user_set[k]

  for k,v in training_user_set.items():
    for i in v:
      training_item_set[i].add(k)
